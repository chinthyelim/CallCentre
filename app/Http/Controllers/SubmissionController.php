...
class SubmissionController extends Controller
{
  ...
  protected function msEscapeString($data) {
    ...
    // Commented out by Chin on 11/02/2020
  ...
  public function list(Request $request)
  {
    ...
    if (strlen($request->input('user_contact_name'))) {
      $sql_where .= "AND b.name LIKE '%" . $this->msEscapeString($request->input('user_contact_name')) . "%' ";
    }
    if (strlen($request->input('ticket_number'))) {
      $sql_where .= "AND a.ticketnumber LIKE '%" . $this->msEscapeString($request->input('ticket_number')) . "%' ";
    }
    ...
    switch ($request->post('search_type')) {
      case 'user_contact_names':
        $results = DB::select(
          ...
          FROM users a
          WHERE a.name LIKE '%" . $this->msEscapeString($request->post('search_string')) . "%'
          ORDER BY a.name
          LIMIT 10
          "
        );
      ...
      case 'ticket_numbers':
        $results = DB::select(
          ...
          FROM submissions a
          WHERE a.ticketnumber LIKE '%" . $this->msEscapeString($request->post('search_string')) . "%'
